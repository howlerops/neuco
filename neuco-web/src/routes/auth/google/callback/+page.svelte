<script lang="ts">
	...
	onMount(async () => {
		...
		if (errorParam) {
			status = 'error';
			errorMessage = $page.url.searchParams.get('error_description') ?? 'Google authorization failed.';
			return;
		}
		...
		try {
			...
			const data = await apiClient.post<CallbackResponse>('/api/v1/auth/google/callback', {
				code,
				redirectUri
			});
