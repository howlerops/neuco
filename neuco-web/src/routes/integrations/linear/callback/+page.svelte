<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			await apiClient.post(`/api/v1/projects/${projectId}/linear/callback`, {
				code,
				state
			});
			status = 'success';
			...
			status = 'error';
			errorMessage = err instanceof Error ? err.message : 'Failed to connect Linear.';
		}
	});
</script>

<svelte:head>
	<title>Connecting Linear — Neuco</title>
</svelte:head>
